...
export function addGround(app) {
  ...
  const ground = new PIXI.Graphics().rect(0, groundY - groundHeight, width, groundHeight).fill({ color: 0xdddddd });
  ...
  for (let index = 0; index < plankCount; index++) {
    ...
    const plank = new PIXI.Graphics().rect(0, plankY - plankHeight, plankWidth, plankHeight).fill({ color: 0x241811 });
  ...
  const rail = new PIXI.Graphics().rect(0, railY - railHeight, width, railHeight).fill({ color: 0x5c5c5c });
